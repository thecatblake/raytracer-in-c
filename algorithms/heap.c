...
int heap_extract(Heap* heap, void** data) {
    ...
    *data = heap->tree[0];
    ...
    save = heap->tree[heap_size(heap) - 1]; // copy the last element
    ...
    heap->tree[0] = save; // set the last element to the top element


    // push down the top to appropriate position
    ipos = 0;
    while(1) {
        lpos = heap_left(ipos);
        ...
        if(mpos == ipos) // the heap property has been restored
            break;
        else {
            // swap to the bigger one
            temp = heap->tree[mpos];
